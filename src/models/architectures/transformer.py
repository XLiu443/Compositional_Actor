...
class Encoder_TRANSFORMER(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        self.skelEmbedding = nn.Linear(self.input_feats, self.latent_dim*2)
        
        self.sequence_pos_encoder = PositionalEncoding(self.latent_dim*2, self.dropout)
        
        # self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))
        
        seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=self.latent_dim*2,
    ...
    def forward(self, batch):
        ...
        x, y1, y2, mask, alphas = batch["x"], batch["y1"], batch["y2"], batch["mask"], batch["alphas"]     #x(24*24*6*60)是video, y1,y2是原始数据的action+合成数据的action, y1, y2的shape都是24
       ...
       #     mix_muquery = alphas*self.muQuery[y1] + (1-alphas)*self.muQuery[y2]
            mix_muquery = torch.cat( (self.muQuery[y1], self.muQuery[y2]), axis = 1)                 #self.muQuery[y1]的维度24*256，mix_muquery的维度24*512
 
       #     mix_sigmaquery = alphas*self.sigmaQuery[y1] + (1-alphas)*self.sigmaQuery[y2]
            mix_sigmaquery =  torch.cat( (self.sigmaQuery[y1], self.sigmaQuery[y2]), axis = 1)       #self.sigmaQuery[y1]的维度24*256, mix_sigmaquery的维度24*512
            xseq = torch.cat((mix_muquery[None], mix_sigmaquery[None], x), axis=0)                   #xseq in shape 62*24*512
...
class Decoder_TRANSFORMER(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        seqTransDecoderLayer = nn.TransformerDecoderLayer(d_model=self.latent_dim*2,
                                                          nhead=self.num_heads,
                                                          dim_feedforward=self.ff_size,
                                                          dropout=self.dropout,
        ...
        self.finallayer = nn.Linear(self.latent_dim*2, self.input_feats)
        
    def forward(self, batch):
        ipdb.set_trace()
        #z, y, mask, lengths = batch["z"], batch["y"], batch["mask"], batch["lengths"]
        z, y1, y2, mask, lengths, alphas = batch["z"], batch["y1"], batch["y2"], batch["mask"], batch["lengths"], batch["alphas"]    #z(24*512)是latent, y1, y2和encoder一样
        ...
        if self.ablation == "zandtime":
           ...
           #     z_action = alphas*self.actionBiases[y1] + (1-alphas)*self.actionBiases[y2]
                z_action = torch.cat( (self.actionBiases[y1], self.actionBiases[y2]), axis = 1)      #self.actionBiases[y1]in shape 24*256, z_action in shape 24*512
                z = z + z_action                   #z in shape 24*512
