...
class Encoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.ablation == "average_encoder":
            ...
            mix_muquery = torch.cat( (self.muQuery[y], self.muQuery[y]), axis = 1)                  # self.muQuery的维度12*256, y是action, self.muQuery[y]的维是20*256
            mix_sigmaquery =  torch.cat( (self.sigmaQuery[y], self.sigmaQuery[y]), axis = 1)        # self.sigmaQuery的维度12*256, self.sigmaQuery[y]的维度是20*256
            xseq = torch.cat((mix_muquery[None], mix_sigmaquery[None], x), axis=0)                  # mix_muquery的维度是20*512, mix_sigmaquery的维度是20*512,x的维度是60*20*512, xseq的维度是62*20*512
...
#        pdb.set_trace()
        ...
        if self.ablation == "zandtime":
            ...
            if self.ablation == "concat_bias":
                ...
                z_action = torch.cat( (self.actionBiases[y], self.actionBiases[y]), axis = 1)        #self.actionBiases的维度12*256, self.actionBiases[y]的维度是20*256
                z = z + z_action                                                                     #z的维度20*512, z_action维度20*512
              #  z = z + self.actionBiases[y]
                z = z[None]  # sequence of size 1                                                    #扩展z的维度到1*20*512
