...
from src.utils.tensors import collate, collate_function
...
def do_epochs(model, datasets, parameters, optimizer, writer):
 ...
 #                               shuffle=True, num_workers=0, collate_fn=collate_function)
    train_iterator = DataLoader(dataset, batch_size=parameters["batch_size"],
...
#    checkpointpath = os.path.join("checkpoint_5000.pth.tar")
#    state_dict = torch.load(checkpointpath, map_location=model.device)
